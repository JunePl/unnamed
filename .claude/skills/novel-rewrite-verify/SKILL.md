...
4. **确切组合 grep -Hn 可复现**：随机抽 3-5 个短语对**十七本全池** `grep -Hn` 跑——确切组合（非近义/同词根）是否真在所标那本那一行？「没停」标「没停下」、书名标错（终局标成玩乐）、查无此句，都退回。
...
REF="$D/终局委员会.txt $D/玩乐关系.txt $D/术师手册_utf8.txt $D/弑神者.txt \
     $D/骑士之潭.txt $D/魔战公主.txt $D/天使大人.txt $D/药师少女的独语.txt \
     $D/世界末日的世界录.txt $D/优雅事件簿.txt $D/侦探之死.txt $D/地下城.txt \
     $D/沉默魔女.txt $D/爱之昏君.txt $D/神的记事本.txt $D/空之境界.txt \
     $D/魔法师联社.txt"
# ⛔ 池内不含本作正文——拿本作正文当语感锚＝复制污染，与 novel-rewrite 铁律冲突。
# ⛔ 也不含一世之尊（停用）。全池搜，不许先挑书。
echo "REF=\"$REF\""
