...
### ⭐⭐⭐ Layer 0：段落骨架核对（最高优先级·人工·先于一切 grep）

**骨架 = 整段的「情绪推进逻辑 + 修辞组合顺序」，不是单句句式。这是防 AI 味的最高一级，grep 查不出来，必须人工核对。**

逐段检查：
1. 这段是否标明了"参考母段"（来自哪个作品哪一处的真实整段）？**没有母段 = 直接判废，退回重写，不必再往下跑 grep。**
2. 本作这段的「情绪推进逻辑 + 修辞顺序」是否和母段**一致**？（例：母段是"前后落差自嘲 → 夸张类比收尾"，本段是否也是这个推进？）
3. 还是说本段只是单句都 grep 过、整段的推进逻辑却是 AI 自己拼的？（这是最隐蔽的 AI 味，必须揪出）

4. **⭐⭐⭐ 片段级零填充核对（最严）**：成稿是否每个片段（短语级）都列出了母句出处？有没有"标几个母段当幌子、其余句子自由写"的偷工（如约纳斯死亡段曾 13 句只 4 句有母段）？随机抽查成稿里的画面句/短语，要求指出它的母句出处——**指不出＝自由填充＝退回重写**。

**输出**：逐段列【声称的母段】+【是否真的同骨架】+【片段是否零填充·随机抽查结果】+【判定：通过 / 退回】。详见 [[feedback_skeleton_first]] 第〇铁律 + 执行细则第 1 条。
